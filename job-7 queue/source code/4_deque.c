...
// fungsi delete_rear() digunakan untuk menghapus elemen daribelakang
void delete_rear(){
    if((f==-1) && (r==-1)){
        printf("Deque kosong");
    }else if(f==r){
        ...
        printf("\nElemen yang dihapus adlah %d ", deque[r]);
        r=r-1;
    }
}
int main(){
    ...
    insert_rear(50);
    insert_rear(80);
    display(); // Memanggil fungsi display untuk mendapatkan nilyang di deque
    getfront(); // Mendapatkan nilai pada bagian front-end
    getrear(); // Mendapatkan nilai pada bagian rear-end
    deleteFront();
    delete_rear();
    display(); // Memanggil fungsi display untuk mendapatkan nilaisetelah proses penghapusan
    return 0;
}