...
void heapify(int array[], int ukuran, int i) {
    ...
    // Menemukan yang terbesar di antara root, left child danright chi
        int largest = i;
        int l = 2 * i + 1;
        int r = 2 * i + 2;
        ...
        if (r < ukuran && array[r] > array[largest])
            largest = r;
        // Tukar (swap) dan lanjutkan proses untuk mengatur ulangelemen heap jika root bukan yang terbesar
        if (largest != i) {
